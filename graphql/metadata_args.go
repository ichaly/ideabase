...
func (my *Metadata) orderOption() error {
	for k, v := range my.Nodes {
		...
		name := utility.JoinString(k, SUFFIX_SORT_INPUT)
		sort := &internal.Class{
			Name:   name,
...
func (my *Metadata) whereOption() error {
	for k, v := range my.Nodes {
		...
		name := utility.JoinString(k, SUFFIX_WHERE_INPUT)
		...
		for _, f := range v.Fields {
			...
			where.Fields[f.Name] = &internal.Field{
				Name: f.Name,
				Type: ast.NamedType(utility.JoinString(f.Type.Name(), SUFFIX_EXPRESSION), nil),
			}
		}
		my.Nodes[name] = where
	}
	...
	for k, v := range my.Nodes {
		...
		for _, suffix := range list {
			class := &internal.Class{
				Kind:   ast.InputObject,
				Name:   utility.JoinString(k, suffix),
				Fields: make(map[string]*internal.Field),
			}
			for _, f := range v.Fields {
				...
				if !slice.Contain(scalars, f.Type.Name()) {
					...
					kind = ast.ListType(ast.NamedType(utility.JoinString(f.Type.Name(), suffix), nil), nil)
				}
