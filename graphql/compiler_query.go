...
func (my *compilerContext) renderSelect(id, pid int, f *ast.Field) {
	...
	alias := utility.JoinString(table, "_", convertor.ToString(id))
	my.Write(` SELECT `)
...
func (my *compilerContext) renderDistinct(id, pid int, f *ast.Field) {
	...
	for i, v := range list {
		...
		my.Quoted(utility.JoinString(field.Table, "_", convertor.ToString(id)))
		my.Write(".")
		my.Quoted(field.Column)
	}
	my.Write(`) `)
...
func (my *compilerContext) renderInner(id, pid int, f *ast.Field) {
	...
	my.Write(` = `)
	my.Quoted(utility.JoinString(join.TableRelation, "_", convertor.ToString(pid)))
	my.Write(` . `)
...
func (my *compilerContext) renderWhere(id, pid int, f *ast.Field) {
	...
	if ok && field.Link != nil {
		...
		if field.Kind == MANY_TO_MANY {
			...
			relation = utility.JoinString(link.TableRelation, "_", convertor.ToString(pid))
		}
		value := &ast.Value{Kind: ast.ObjectValue, Children: []*ast.ChildValue{{
			Name: utility.JoinString(`"`, link.TableName, `"."`, link.ColumnName, `"`),
			Value: &ast.Value{Kind: ast.ObjectValue, Children: []*ast.ChildValue{{
				Name: EQ, Value: &ast.Value{
					Kind: ast.BlockValue,
					Raw:  utility.JoinString(`"`, relation, `"."`, link.ColumnRelation, `"`),
				},
			}}},
		}}}
		//拼接原始条件
...
func (my *compilerContext) renderRecursiveSelect(id, pid int, f *ast.Field) {
	...
	alias := utility.JoinString("__rcte_", table)
	...
	if "children" == f.Name {
		key := utility.JoinString(`"`, table, `"."`, column, `"`)
		//条件1
		children = append(children, my.buildChildValue(key, IS, "NOT_NULL", ast.EnumValue))
		//条件2
		children = append(children, my.buildChildValue(key, NE, utility.JoinString(`"`, field.Link.TableRelation, `"."`, field.Link.ColumnRelation, `"`), ast.BlockValue))
		//条件3
		children = append(children, my.buildChildValue(key, EQ, utility.JoinString(`"`, alias, `"."`, field.Link.ColumnRelation, `"`), ast.BlockValue))
	} else {
		key := utility.JoinString(`"`, alias, `"."`, field.Link.ColumnRelation, `"`)
		//条件1
		children = append(children, my.buildChildValue(key, IS, "NOT_NULL", ast.EnumValue))
		//条件2
		children = append(children, my.buildChildValue(key, NE, utility.JoinString(`"`, alias, `"."`, field.Link.ColumnName, `"`), ast.BlockValue))
		//条件3
		children = append(children, my.buildChildValue(key, EQ, utility.JoinString(`"`, field.Link.TableName, `"."`, field.Link.ColumnName, `"`), ast.BlockValue))
	}
